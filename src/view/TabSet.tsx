...
export interface ITabSetProps {
    ...
    titleFactory?: (
        node: TabNode,
    ) => ITitleObject | React.ReactNode | undefined;
    icons: IIcons;
    editingTab?: TabNode;
    path?: string;
}
...
export const TabSet = (props: ITabSetProps) => {
    ...
    const isTabStretch =
        node.isEnableSingleTabStretch() && node.getChildren().length === 1;
    const showClose =
        (isTabStretch && (node.getChildren()[0] as TabNode).isEnableClose()) ||
        node.isEnableClose();
        ...
        if (typeof icons.more === "function") {
            ...
            overflowContent = (
                <>
                    ...
                    <div
                        className={cm(
                            CLASSES.FLEXLAYOUT__TAB_BUTTON_OVERFLOW_COUNT,
                        )}
                    >
                        {hiddenTabs.length}
                    </div>
    ...
    if (
        selectedTabNode !== undefined &&
        layout.isSupportsPopout() &&
        selectedTabNode.isEnableFloat() &&
        !selectedTabNode.isFloating()
    ) {
    ...
    if (!node.isMaximized() && showClose) {
        const title = isTabStretch
            ? layout.i18nName(I18nLabel.Close_Tab)
            : layout.i18nName(I18nLabel.Close_Tabset);
        const btns = showHeader ? headerButtons : buttons;
        btns.push(
            <button
                key="close"
                ...
                onTouchStart={onInterceptMouseDown}
            >
                {typeof icons.closeTabset === "function"
                    ? icons.closeTabset(node)
                    : icons.closeTabset}
            </button>,
        );
    }
    ...
    tabStripClasses +=
        " " + CLASSES.FLEXLAYOUT__TABSET_TABBAR_OUTER_ + node.getTabLocation();
    ...
    if (showHeader) {
        ...
        header = (
            ...
            >
                <div className={cm(CLASSES.FLEXLAYOUT__TABSET_HEADER_CONTENT)}>
                    {headerContent}
                </div>
                {headerToolbar}
            </div>
        ...
        >
            ...
            >
                <div
                    style={{
                        left: position,
                        width: isTabStretch ? "100%" : "10000px",
                    }}
                    className={
                        cm(
    ...
    const center = (
        <div className={cm(CLASSES.FLEXLAYOUT__TABSET_CONTENT)}>
            {placeHolder}
        </div>
    );
